...
A collection of agent skills (slash commands and behaviors) loaded by opencode from `SKILL.md` files in `.opencode/skills/`, `.agents/skills/`, or `.claude/skills/` (Claude-compatible path). Skills are organized into buckets and consumed by per-repo configuration emitted by `/setup-matt-pocock-skills`.
