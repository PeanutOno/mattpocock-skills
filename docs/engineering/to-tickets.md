...
Before slicing, `to-tickets` looks for prefactoring — "make the change easy, then make the easy change" — and orders that work first. In opencode, the quiz is staged into one structured question at a time — shape first (with slices recomputed from each answer), then blocking edges against that shape, then approval — and nothing publishes until each step has your answer. Blockers go first so each ticket's "Blocked by" can reference a real ticket.
